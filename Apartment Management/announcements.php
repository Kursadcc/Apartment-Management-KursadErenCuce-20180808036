...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>{$row['contentId']}</td><td>{$row['announceDate']}</td><td >{$row['content']}</td><td style='width: 50px;'><form action='announcements.php' method='post'><input class='btn btn-primary' type='submit' name='delete' value='Delete' /><input type='hidden' name='id' value='".$row["contentId"]."'/></form></td></tr></br>";
    }
...
$connection->close();
 ...
 ?>
  <button style="margin-left: 77.3%;" onclick="location.href='addAnnouncement.php'" class="btn btn-primary">Add</button>
