...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <button style="margin-left:81.5%;" onclick="location.href='addAnnouncement.php'" class="btn btn-success">+Add</button>
  <div style="width:70%;margin-left:15%;background-color:rgb(240,240,240);" >
  <table style="padding:7px;"  class="table table-striped table-hover ">
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>{$row['announceDate']}</td><td >{$row['content']}</td><td style='width: 50px;'><form action='announcements.php' method='post'><input class='btn btn-danger' type='submit' name='delete' value='Delete' /><input type='hidden' name='id' value='".$row["contentId"]."'/></form></td></tr>";
    }
